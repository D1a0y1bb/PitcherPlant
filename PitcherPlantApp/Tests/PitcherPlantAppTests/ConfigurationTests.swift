...
func releaseWorkflowPublishesAdHocArtifactsWhenSigningSecretsAreMissing() throws {
    ...
    #expect(workflow.contains("if [ -n \"${{ steps.release.outputs.bundle_marketing_version }}\" ]; then"))
    #expect(workflow.contains("RELEASE_BUILD_SETTINGS+=(\"MARKETING_VERSION=${{ steps.release.outputs.bundle_marketing_version }}\")"))
    ...
    #expect(workflow.contains("STABLE_UPDATE_CHECK_URL=\"https://github.com/D1a0y1bb/PitcherPlant/releases/latest/download/appcast.xml?cachebust=1\""))
    #expect(workflow.contains("BETA_UPDATE_CHECK_URL=\"https://github.com/D1a0y1bb/PitcherPlant/releases/download/appcast-beta/appcast.xml\""))
    #expect(workflow.contains("release_flags+=(--prerelease --latest=false)"))
    #expect(workflow.contains("gh release edit \"$CHANNEL_TAG\" --prerelease --latest=false"))
    #expect(workflow.contains("gh release upload \"$CHANNEL_TAG\""))
