...
| `Persistence/` | GRDB 数据库和旧数据迁移 |
| `Support/` | 工作区定位、本地化、主题、报告过滤 |
| `Tests/PitcherPlantAppTests/` | 摄取、分析器、报告、数据库、迁移测试 |
...
首次启动时，`MigrationService` 会按以下顺序导入旧数据：

1. `LegacyData/LegacyImport/.pitcherplant-web-state.json`
2. `.pitcherplant-web-state.json`
3. `LegacyData/LegacyImport/reports/`
4. `reports/`
5. `LegacyData/LegacyImport/PitcherPlant.sqlite`
6. `PitcherPlant.sqlite`
...
- 已落地：批量提交包导入、队列串行执行、失败任务重试、证据复核持久化、证据级风险聚合、扩展输入解析、扩展导出、白名单建议、候选召回、可选审计辅助解释、CI 和发布 workflow。
- 待完善：证据查看器高亮与跳转、代码 diff 视图、图片左右对比、指纹包导入导出、标签清理、跨批次复用图谱、校准 fixture 和性能指标归档。
